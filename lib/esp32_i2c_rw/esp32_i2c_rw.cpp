    ...
    uint8_t *data)
{
    ...
    if (size > 1)
        i2c_master_read(cmd, data, size - 1, i2c_ack_type_t::I2C_MASTER_ACK);

    i2c_master_read_byte(cmd, data + size - 1, i2c_ack_type_t::I2C_MASTER_NACK);
